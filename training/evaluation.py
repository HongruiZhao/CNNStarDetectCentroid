...
def trilateration_centroid(dist_map, seg_map, radius, pixel_size=6/1000.0):
    ...
    dist_min = ndimage.minimum_filter(dist_map, size=(10,10), mode='constant')
    mask = (dist_map == dist_min)

    for current_row in range(image_height):
        for current_col in range(image_width):
            if dist_map[current_row, current_col] <= threshold and seg_map[current_row, current_col] > 0 and mask[current_row, current_col]:
